...
echo "==============================================="
echo "OpenVPN Protocol Switch Tool (UDP to TCP)"
echo "==============================================="
...
read -p "3. Enter TCP port to use (press enter to keep $CURRENT_PORT): " NEW_PORT
NEW_PORT=${NEW_PORT:-$CURRENT_PORT}

# Update the configuration file
echo "4. Updating server configuration to use TCP protocol..."
...
if command -v ufw > /dev/null; then
    if ufw status | grep -q "active"; then
        echo "   UFW firewall detected. Adding TCP rules..."
        ufw allow $NEW_PORT/tcp
        echo "✅ UFW rule added for port $NEW_PORT/tcp"
    fi
elif command -v firewall-cmd > /dev/null; then
    if firewall-cmd --state | grep -q "running"; then
        echo "   FirewallD detected. Adding TCP rules..."
    ...
    if ! iptables -L INPUT -n | grep -q "tcp dpt:$NEW_PORT"; then
        ...
        echo "✅ iptables rule added for port $NEW_PORT/tcp"
